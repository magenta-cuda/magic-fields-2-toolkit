...
function alt_get_audio( $field_name, $group_index=1, $field_index=1, $post_id=NULL, $flag = FALSE ) {
  global $post;
  ...
  if (( strpos( $_SERVER['HTTP_USER_AGENT'], 'iPad;' ) !== FALSE
    || strpos( $_SERVER['HTTP_USER_AGENT'], 'iPhone;' ) !== FALSE ) || $flag ) {
    ...
    if ( array_key_exists( $extension, $mime_type ) ) {
      ...
      $player = 'Your browser cannot play ' . $extension . ' audio files.';
    }
  } else {
