...
/*
    This search widget can search for posts by the value of custom fields, taxonomies and post content. The widget uses user friendly substitutions for the
...
class Search_Using_Magic_Fields_Widget extends WP_Widget {
    ...
    const OPTIONAL_MAXIMUM_VALUE_SUFFIX     = '-stcfw-maximum-value';          # inputs for a numeric search fields
    const SEARCH_RESULTS_FIELDS_AND_FILTERS = 'magic_fields_2_toolkit_search_results_fields_and_filters';
    const SEARCH_RESULTS_TEMPLATE_PREFIX    = 'search-result-template-for-';   # prefix for search result template post name
...
EOD;
    public static $TEMPLATE_NOT_FOUND;

	public function __construct( ) {
        global $mf_domain;
		parent::__construct( 'search_magic_fields', __( 'Search using Magic Fields', $mf_domain ),
            [ 'classname' => 'search_magic_fields_widget', 'description' => __( 'Search for Custom Posts using Magic Fields', $mf_domain ) ]
        );
        self::$TEMPLATE_NOT_FOUND  = __( 'no template found - the generic will be used', $mf_domain );
	}
...
<option value="no-selection">--<?php _e( 'select post type', $mf_domain ); ?>--</option>
<?php
        # get data for the administrator selected post types
        $selected_types = '"' . implode( '", "', array_diff( array_keys( $instance ),
            [ 'maximum_number_of_items', 'set_is_search', 'enable_table_view_option', 'table_shortcode', 'table_width' ] ) ) . '"';
        $SQL_LIMIT = self::SQL_LIMIT;
        $types = $wpdb->get_results( <<<EOD
SELECT post_type, COUNT(*) count FROM $wpdb->posts WHERE post_type IN ( $selected_types ) AND post_status = "publish"
...
<div>
<?php _e( 'Results should satisfy', $mf_domain ); ?><br> 
<input type="radio" name="magic-fields-search-and-or" value="and" checked><strong><?php _e( 'All', $mf_domain ); ?></strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<input type="radio" name="magic-fields-search-and-or" value="or"         ><strong><?php _e( 'Any', $mf_domain ); ?></strong></br>
<?php _e( 'of the search conditions', $mf_domain ); ?>.
</div>
<?php
...
<div class="scpbcfw-admin-search-fields-help">
    <a href="http://magicfields17.wordpress.com/magic-fields-2-search-0-4-1/#administrator" target="_blank"><?php _e( 'help', $mf_domain ); ?></a>
</div>
<h4 class="scpbcfw-admin-search-fields-title"><?php _e( 'Select Search Fields and Content Macro Display Fields for', $mf_domain ); ?>:</h4>
...
<?php
        ...
        $SQL_LIMIT = self::SQL_LIMIT;
        $types = $wpdb->get_results( <<<EOD
SELECT post_type, COUNT(*) count FROM $wpdb->posts WHERE post_type IN ( $mf2_types ) AND post_status = "publish"
    GROUP BY post_type ORDER BY count DESC LIMIT $SQL_LIMIT
EOD
            , OBJECT_K );
...
SELECT post_type, taxonomy, count(*) count
    FROM ( SELECT p.post_type, x.taxonomy, r.object_id FROM $wpdb->term_relationships r, $wpdb->term_taxonomy x, $wpdb->terms t, $wpdb->posts p
        WHERE r.term_taxonomy_id = x.term_taxonomy_id AND x.term_id = t.term_id AND r.object_id = p.ID AND p.post_type IN ( $mf2_types )
...
<div class="scpbcfw-admin-search-fields-display-button"><?php _e( 'Open', $mf_domain ); ?></div>
...
EOD
            ...
            $previous = array_filter( array_merge(
                !empty( $instance['tax-order-' . $name] ) ? explode( ';', $instance['tax-order-' . $name] ) : [ ],
                !empty( $instance['order-' . $name] )     ? explode( ';', $instance['order-' . $name] )     : [ ]
            ) );
...
?>
    <input type="checkbox" class="mf2tk-selectable-field" id="<?php echo $this->get_field_id( $name ); ?>"
        name="<?php echo $this->get_field_name( $name ); ?>[]" value="<?php echo $tax_name; ?>"
        <?php if ( $selected && in_array( $tax_name, $selected ) ) { echo ' checked'; } ?>>
    <input type="checkbox" id="<?php echo $this->get_field_id( 'show-' . $name ); ?>" class="scpbcfw-select-content-macro-display-field"
        name="<?php echo $this->get_field_name( 'show-' . $name ); ?>[]" value="<?php echo $tax_name; ?>"
...
?>
    ...
    <input type="checkbox" id="<?php echo $this->get_field_id( 'show-' . $name ); ?>" name="<?php echo $this->get_field_name( 'show-' . $name ); ?>[]"
        <?php if ( $field->type !== 'multiline' && $field->type !== 'markdown_editor' ) { echo 'class="scpbcfw-select-content-macro-display-field"'; } ?>
        value="<?php echo $meta_key; ?>" <?php if ( $show_selected && in_array( $meta_key, $show_selected ) ) { echo ' checked'; } ?>
        <?php if ( ( $instance && !isset( $instance['enable_table_view_option'] ) ) || $field->type === 'multiline' || $field->type === 'markdown_editor' ) {
            echo 'disabled'; } ?>>
...
<input type="number" min="0" max="1024" id="<?php echo $this->get_field_id( 'maximum_number_of_items' ); ?>"
    name="<?php echo $this->get_field_name( 'maximum_number_of_items' ); ?>" class="scpbcfw-admin-search-fields-option-number"
    value="<?php echo isset( $instance['maximum_number_of_items'] ) ? $instance['maximum_number_of_items'] : 16; ?>" size="4">
<?php _e( 'Maximum number of items to display per custom field', $mf_domain ); ?>:
<div style="clear:both;"></div>
</div>
<div class="scpbcfw-admin-search-fields-option">
...
<div class="scpbcfw-admin-search-fields-option">
<input type="checkbox" id="<?php echo $this->get_field_id( 'enable_table_view_option' ); ?>"
    name="<?php echo $this->get_field_name( 'enable_table_view_option' ); ?>" class="scpbcfw-admin-search-fields-option-checkbox"
    value="table view option enabled" <?php if ( !$instance || isset( $instance[ 'enable_table_view_option' ] ) ) { echo 'checked'; } ?>>
<?php _e( 'Enable option to display search results using a content template', $mf_domain ); ?>:
<div style="clear:both;"></div>
</div>
<div class="scpbcfw-admin-search-fields-option">
<input type="number" min="256" max="8192" id="<?php echo $this->get_field_id( 'table_width' ); ?>" name="<?php echo $this->get_field_name( 'table_width' ); ?>"
    class="scpbcfw-admin-search-fields-option-number" <?php if ( !empty( $instance['table_width'] ) ) { echo "value=\"$instance[table_width]\""; } ?>
    <?php if ( $instance && !isset( $instance['enable_table_view_option'] ) ) { echo 'disabled'; } ?> placeholder="from css" size="5">
<?php _e( 'Width in pixels of the container used to display the search results', $mf_domain ); ?>:
<div style="clear:both;"></div>
</div>
<div class="scpbcfw-admin-search-fields-option">
...
?>
<h4 class="scpbcfw-search-fields-conditions-title"><?php _e( 'Specify search conditions', $mf_domain ); ?>:<h4>
<p style="clear:both;margin:0px;">
<?php    
        $taxonomies = [ ];
        $wp_taxonomies = get_taxonomies( '', 'objects' );
        ...
        $fields[ 'pst-std-post_author'  ] = (object) [ 'type' => 'author',    'label' => 'Author'       ];
...
<input id="<?php $meta_key . Search_Using_Magic_Fields_Widget::OPTIONAL_TEXT_VALUE_SUFFIX; ?>"
    ...
    class="scpbcfw-search-fields-for-input" type="text" placeholder="--<?php _e( 'Enter Search Value', $mf_domain ); ?>--">
<?php
                }
?>
</div>
...
<input id="<?php echo $meta_key ?>" name="<?php echo $meta_key ?>" class="scpbcfw-search-fields-for-input" type="text"
    placeholder="--<?php _e( 'Enter Search Value', $mf_domain ); ?>--">
</div>
</div>
<?php
                    continue;
...
SELECT p.post_author, u.display_name, COUNT(*) count FROM $wpdb->posts p, $wpdb->users u
    WHERE p.post_author = u.ID AND p.post_type = %s AND p.post_status = "publish" AND p.post_author IS NOT NULL GROUP BY p.post_author ORDER BY count
EOD
...
<?php
                    ...
                    if ( $count === $SQL_LIMIT ) {
                        # too many values to display so also let user manually enter a search value
?>
<input type="text" id="pst-std-post_author<?php echo Search_Using_Magic_Fields_Widget::OPTIONAL_TEXT_VALUE_SUFFIX; ?>"
    ...
    class="scpbcfw-search-fields-for-input" placeholder="--<?php _e( 'Enter Search Value', $mf_domain ); ?>--">
<?php
                    }
?>
</div>
...
EOD
                ...
                $values = [ ];   # to be used by serialized fields
                $numeric = TRUE;
                foreach ( $results as $meta_value => $result ) {
                    ...
                    if ( $field->type === 'related_type' ) {
                        ...
                        if ( $value ) {
                            $value = substr( $value[0], strrpos( $value[0], '/' ) + 1 );
                        } else {
                            $value = '';
                        }
                    ...
                    } else if ( $field->type === 'alt_related_type' || $field->type === 'checkbox_list' || $field->type === 'dropdown'
                        || $field->type === 'alt_dropdown' ) {
...
?>
<h4><?php _e( 'Range Search', $mf_domain );?></h4>
<input id="<?php echo $meta_key . Search_Using_Magic_Fields_Widget::OPTIONAL_MINIMUM_VALUE_SUFFIX; ?>"
    ...
    class="scpbcfw-search-fields-for-input" type="text" placeholder="--<?php _e( 'Enter Minimum Value', $mf_domain ); ?>--">
<input id="<?php echo $meta_key . Search_Using_Magic_Fields_Widget::OPTIONAL_MAXIMUM_VALUE_SUFFIX; ?>"
    ...
    class="scpbcfw-search-fields-for-input" type="text" placeholder="--<?php _e( 'Enter Maximum Value', $mf_domain ); ?>--">
<?php
                }
?>
</div>
...
<?php
    ...
    # the "mf2tk_get_search_result_template_form" AJAX request creates and populates the search result template editor form for a post type
...
<div id="mf2tk-edit-search-result-template-div">
    ...
    <h3 id="mf2tk-edit-search-result-template-h3"><?php _e( 'Create Search Result Template for Post Type', $mf_domain ); ?>: <?php echo $post_type; ?></h3>
...
<?php
        ...
        if ( array_key_exists( $post_type, $field_and_filters ) ) {
            ...
            preg_match( '/field=([\w;]+)\|filter=([\w;]*)(\|width=([^;]*);)?/', $input_value, $matches );
        }
        $fields   = array_key_exists( 1, $matches ) ? $matches[1] : '';
        $filters  = array_key_exists( 2, $matches ) ? $matches[2] : '';
        $width    = array_key_exists( 4, $matches ) ? $matches[4] : '100%';
...
?>
    <div class="mf2tk-field-input-optional">
        <div class="mf2tk-field_value_pane" style="clear:both;">
            <textarea id="mf2tk-search-result-template-textarea" class="mf2tk-how-to-use" rows="10" style="padding:10px;">
...
&lt;div&gt;
&lt;style scoped&gt;
table.mf2tk-<?php echo $post_type; ?>-table{width:<?php echo $width; ?>;border-collapse:collapse;}
...
&lt;/div&gt;
            ...
            <button id="mf2tk-save-search-result-template-button" style="float:right;"><?php _e( 'Save the Content Template', $mf_domain ); ?></button>
        </div>
    </div>
    <!-- optional configuration field -->
    <div class="mf2tk-field-input-optional">
        <h6><?php _e( 'Re-Configure the Content Template', $mf_domain ); ?></h6>
        <div class="mf2tk-field_value_pane" style="clear:both;">
            <fieldset class="mf2tk-configure mf2tk-fields" style="padding:10px;">
...
?>
                <div class="mf2tk-dragable-field">
                    <label class="mf2tk-configure"><input type="checkbox" class="mf2tk-configure" value="<?php echo $field; ?>"<?php echo $checked; ?>>
...
?>
            ...
            <fieldset class="mf2tk-configure mf2tk-filters" style="padding:10px;">
                ...
                <div>
                    ...
                    <label class="mf2tk-configure"><input type="radio" class="mf2tk-configure" name="tk_value_as_image_{$post_type}"
                        value="width"<?php echo $tk_value_as_image_width_checked; ?>><?php _e( 'Width', $mf_domain ); ?></label>
                    <label class="mf2tk-configure"><input type="radio" class="mf2tk-configure" name="tk_value_as_image_{$post_type}"
                        value="height"<?php echo $tk_value_as_image_height_checked; ?>><?php _e( 'Height', $mf_domain ); ?></label>
                ...
                <div>
                    ...
                    <label class="mf2tk-configure"><input type="radio" class="mf2tk-configure" name="tk_value_as_video_{$post_type}"
                        value="width"<?php echo $tk_value_as_video_width_checked; ?>><?php _e( 'Width', $mf_domain ); ?></label>
                    <label class="mf2tk-configure"><input type="radio" class="mf2tk-configure" name="tk_value_as_video_{$post_type}"
                        value="height"<?php echo $tk_value_as_video_height_checked; ?>><?php _e( 'Height', $mf_domain ); ?></label>
                    <label class="mf2tk-configure"><input type="number" class="mf2tk-configure" max="9999"
...
(function(){
    ...
    div.find("button#mf2tk-search-result-template-refresh-button").click(function(e){
        ...
        parent.find("fieldset.mf2tk-configure.mf2tk-styles input[type='text']").each(function(){
            var input=jQuery(this);
            if(input.attr("id")==="mf2tk-table-width"){
                width=input.val().trim();
            }
        });
        var textarea=div.find("textarea#mf2tk-search-result-template-textarea");
        var text=textarea.val();
        text=text.replace(/width:[^;]*;border-collapse:collapse;/,function(match,old){
            return "width:"+width+";border-collapse:collapse;";
        });
...
<?php
    ...
    # This AJAX request gets the search result template for the requested post type
    ...
    add_action( 'wp_ajax_mf2tk_get_search_result_template', function( ) {
        ...
        $template = $wpdb->get_col( $wpdb->prepare ( <<<EOD
SELECT post_content FROM $wpdb->posts WHERE post_type = 'content_macro' AND post_name = '%s' AND post_status = 'publish'
EOD
        , $post_name ) );
        if ( $template ) {
            die( $template[0] );
...
SELECT ID FROM $wpdb->posts WHERE post_type = 'content_macro' AND post_name = '%s' AND post_status = 'publish'
EOD
        , $post_name ) );
        $post = [
            'post_type' => 'content_macro',
            'post_name' => $post_name,
            'post_title' => __( 'Search Result Template for', $mf_domain ) . " $_POST[post_type]",
            'post_status' => 'publish',
        ...
        die( !empty ( $id0 ) ? "Content template $id0 updated." : ( !empty( $id1 ) ? "Content template $id1 created."
...
EOD
        ...
        $results = $wpdb->get_results( $wpdb->prepare( <<<EOD
SELECT u.display_name, u.ID FROM $wpdb->users u, $wpdb->posts p WHERE u.ID = p.post_author AND p.post_type = %s GROUP BY u.ID
EOD
            , $_REQUEST['post_type'] ), OBJECT );
        $author_ids = [ ];
        foreach ( $results as $result ) {
        ...
        foreach ( $_REQUEST as $index => &$request ) {
            if ( $request && substr_compare( $index, Search_Using_Magic_Fields_Widget::OPTIONAL_TEXT_VALUE_SUFFIX, -$suffix_len ) === 0 ) {
        ...
        foreach ( $_REQUEST as $index => &$request ) {
            if ( $request && ( ( $is_min = substr_compare( $index, Search_Using_Magic_Fields_Widget::OPTIONAL_MINIMUM_VALUE_SUFFIX, -$suffix_len ) === 0 )
        ...
        $non_field_keys = [ 'magic_fields_search_form', 'magic_fields_search_widget_option', 'magic_fields_search_widget_number',
            'magic-fields-search-and-or', 'magic-fields-show-using-macro', 'post_type', 'paged' ];
        $sql = '';
        ...
        if ( $sql ) {
            $sql = $wpdb->prepare( "SELECT p.ID FROM $wpdb->posts p WHERE p.post_type = %s AND p.post_status = 'publish' AND ", $_REQUEST['post_type'] )
                . "( $sql )";
        ...
        if ( $sql ) {
            $sql = $wpdb->prepare( "SELECT p.ID FROM $wpdb->posts p WHERE p.post_type = %s AND p.post_status = 'publish' AND ( $sql )",
                $_REQUEST['post_type'] );
...
EOD
        ...
        if ( array_key_exists( 'pst-std-post_author', $_REQUEST ) && $_REQUEST[ 'pst-std-post_author' ] ) {
            ...
            $sql = $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_type = %s AND post_status = 'publish' AND post_author IN ( $authors )",
                $_REQUEST['post_type'] );
    ...
    if ( isset( $_REQUEST[ 'magic-fields-show-using-macro' ] ) && $_REQUEST[ 'magic-fields-show-using-macro' ] === 'use macro' ) {
        ...
        add_action( 'wp_enqueue_scripts', function( ) {
            ...
            if ( file_exists( dirname( __FILE__ ) . "/css/search-results-table-$_REQUEST[post_type].css") ) {
                wp_enqueue_style( 'mf2tk_search_results_table', plugins_url( "css/search-results-table-$_REQUEST[post_type].css", __FILE__ ) );
                ...
                wp_enqueue_style( 'mf2tk_search_results_table', plugins_url( "search-results-table-$_REQUEST[post_type].css", __FILE__ ) );
            } else {
                wp_enqueue_style( 'mf2tk_search_results_table', plugins_url( 'css/mf2tk_search-results-table.css', __FILE__ ) );
            }
        } );
        ...
        add_action( 'template_redirect', function( ) {
            ...
            $fields = ( is_array( $option[ $number ] ) && array_key_exists( $show_fields, $option[ $number ] ) ) ? $option[ $number ][ $show_fields ] : [ ];
            if ( !$fields ) {
            ...
            $macro = $wpdb->get_col( $wpdb->prepare( <<<EOD
SELECT post_content FROM $wpdb->posts WHERE post_type = 'content_macro' AND post_name = '%s' AND post_status = 'publish'
EOD
            , $post_name ) );
            if ( $macro ) { 
            ...
            if ( !empty( $post_type_specific ) ) {
                $content = "[{$options['show_macro']} posts=\"$posts\" a_post=\"$post\"]{$macro}[/{$options['show_macro']}]";
            } else {
                $content = <<<EOD
[{$options['show_macro']} posts="$posts" fields="$fields" a_post="$post" post_type="$_REQUEST[post_type]" table_width="$table_width"]
$macro
[/{$options['show_macro']}]
EOD;
            }
            $content = "<div class=\"scpbcfw-search-result-container\">$content</div>";
