...
trait Magic_Fields_2_Toolkit_Post_Filter {
    ...
    # example: get_posts_with_spec( 'carburetor:year:-1969,1970,-1971,1972;company_type:llc,-corporation,;' );
    
    static function get_posts_with_spec( $spec ) {
        global $wpdb;
        error_log( 'get_posts_with_spec():$spec=' . $spec );
        ...
        if ( !preg_match( '/^([a-z_-]+)(:(((([a-z_-]+):(((-)?[a-z_-]+)(,)?)+)(;)?)+))?(#(\d+))?$/', $spec, $matches ) ) {
            return [ ];
        }
        ...
        foreach ( $tax_matches as $tax_match ) {
            ...
            $exclude_tags = array_filter( $tags, function ( $value ) {
                return substr_compare( $value, '-', 0, 1 ) === 0;
            } );
            # subtract exclude tags to get include tags
            $include_tags = array_diff( $tags, $exclude_tags );
            # remove - from exclude tags
            $exclude_tags = array_map( function( $value ) {
                return substr( $value, 1 );
            }, $exclude_tags );
            $include_exclude_tags = [ $include_tags, $exclude_tags ];
            foreach ( $include_exclude_tags as &$tags ) {
                ...
                foreach ( $tags as $tag ) {
                    ...
                    $tag_sql .= $wpdb->prepare( ' t.slug = %s ', $tag );
                }
                if ( $tag_sql ) {
                    $sqls[ ] = $wpdb->prepare( ' ( x.taxonomy = %s', $taxonomy ) . " AND ( $tag_sql ) ) ";
                }
                unset( $sqls );
            }
        }   # foreach ( $tax_matches as $tax_match ) {
        $sql_prefix = $wpdb->prepare( <<<EOD
        ...
        if ( $include_sqls ) {
            foreach ( $include_sqls as $include_sql ) {
                ...
                if ( !$includes ) {
                    ...
                    $includes = array_intersect( $includes, $new_includes );
                }
            }
        } else {
            $includes = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_type = %s", $post_type ) );
        }
        if ( $exclude_sqls ) {
            $excludes = [ ];
